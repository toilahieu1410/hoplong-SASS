...
const Calceled = ({navigation, number}) => {
    const dispatch = useDispatch();
    const listDonHang = useSelector((store) => store.donHang.listDonHang);
    const [statusCode, setStatusCode] = useState('canceled');
         ...
         renderItem={({item, index}) => (
           <View style={[styles.container, {marginTop: 10}]}>
             <View style={styles.mb}>
               <Text style={styles.tracking} onPress={() => navigation.navigate('ChiTietDonHang', {data: item.id})}>{item.tracking}</Text>
               <View style={styles.flex}>
                 ...
                 <View style={styles.detail}>
                   ...
                   <Text style={[styles.value,{fontWeight:'bold',color:'#004C8A'}]}>
                     {item.total
                       .toString()
                       .replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1.')}{' '}
                     đ | <Text style={[styles.value,{fontWeight:'normal'}]}>{item.products_sum_quantity} sản phẩm</Text>
                   </Text>
                 </View>
               </View>
             </View>
